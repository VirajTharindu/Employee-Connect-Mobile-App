...
class _PoliceScreenState extends State<PoliceScreen> {
  ...
  Map<String, List<FamilyMember>> groupedPoliceFamilies =
  ...
  Future<List<FamilyMember>> _fetchPoliceFamilies() async {
    final List<FamilyMember> familyMembers =
        await locator.familyRepository.queryPoliceEmployees();

    groupedPoliceFamilies = {}; // Clear the map before grouping
  ...
  Future<void> generatePdf() async {
    ...
    final List<FamilyMember> members = groupedPoliceFamilies.values
    ...
    if (members.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No Police Employees found to generate PDF'),
          ...
          return pw.Container(
            ...
            child: pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Police Employees - Employee Connect App",
                    style: pw.TextStyle(
          ...
          return [
            ...
            pw.Container(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Total Households: ${groupedPoliceFamilies.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                  pw.Text(
                    "Total Police Employees: ${members.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                ],
            ...
            pw.ListView.builder(
              itemCount: groupedPoliceFamilies.length,
              itemBuilder: (context, index) {
                final householdNumber =
                    groupedPoliceFamilies.keys.toList()[index];
                final householdMembers =
                    groupedPoliceFamilies[householdNumber]!;
    ...
    try {
      ...
      final currentDate = DateTime.now();
      final fileName =
          "Police_Employees_Employee_Connect_${currentDate.day}-${currentDate.month}-${currentDate.year}.pdf";
          ...
          children: [
            ...
            FutureBuilder<List<FamilyMember>>(
              ...
              builder: (context, snapshot) {
                ...
                // Calculate family and member counts after data is fetched
                int familyCount = groupedPoliceFamilies.keys.length;
                int memberCount = groupedPoliceFamilies.values
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const NoDataFallback(
              ...
              message: 'There are no family members recorded under the Police job category.',
            );
          }
