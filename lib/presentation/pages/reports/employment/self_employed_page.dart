...
import 'package:employee_connect/presentation/widgets/no_data_fallback.dart';
...
class _SelfEmployedScreenState extends State<SelfEmployedScreen> {
  ...
  Map<String, List<FamilyMember>> groupedSelfEmployedFamilies =
      {}; // Initialize to avoid late initialization error

  int familyCount = 0; // Initialize family count
  int memberCount = 0; // Initialize member count
  ...
  Future<List<FamilyMember>> _fetchSelfEmployedFamilies() async {
    final List<FamilyMember> familyMembers =
        await locator.familyRepository.querySelfEmployedEmployees();

    // Group family members by household number
    Map<String, List<FamilyMember>> tempGroupedFamilies = {};
    for (var familyMember in familyMembers) {
      if (tempGroupedFamilies.containsKey(familyMember.householdNumber)) {
        tempGroupedFamilies[familyMember.householdNumber]!.add(familyMember);
      } else {
        tempGroupedFamilies[familyMember.householdNumber] = [familyMember];
      }
    }

    // Update the state with the grouped families and recalculate counts
    setState(() {
      groupedSelfEmployedFamilies = tempGroupedFamilies;
      familyCount = groupedSelfEmployedFamilies.keys.length;
      memberCount = groupedSelfEmployedFamilies.values
  ...
  Future<void> generatePdf() async {
    ...
    final List<FamilyMember> members = groupedSelfEmployedFamilies.values
    ...
    if (members.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No Self-Employees found to generate PDF'),
          ...
          return pw.Container(
            ...
            child: pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Self Employees - Employee Connect App",
                    style: pw.TextStyle(
          ...
          return [
            ...
            pw.Container(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Total Households: ${groupedSelfEmployedFamilies.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                  pw.Text(
                    "Total Self Employees: ${members.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                ],
            ...
            pw.ListView.builder(
              itemCount: groupedSelfEmployedFamilies.length,
              itemBuilder: (context, index) {
                final householdNumber =
                    groupedSelfEmployedFamilies.keys.toList()[index];
    ...
    try {
      ...
      final currentDate = DateTime.now();
      final fileName =
          "Self_Employees_Employee_Connect_${currentDate.day}-${currentDate.month}-${currentDate.year}.pdf";
      ...
      body: FutureBuilder<List<FamilyMember>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const NoDataFallback(
              title: 'No Employees Found',
              message: 'There are no family members recorded as Self-Employed.',
            );
          }
