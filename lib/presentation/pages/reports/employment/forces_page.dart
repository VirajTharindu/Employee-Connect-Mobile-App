...
class _ForcesScreenState extends State<ForcesScreen> {
  ...
  Map<String, List<FamilyMember>> groupedForceFamilies =
  ...
  Future<List<FamilyMember>> _fetchForceFamilies() async {
    final List<FamilyMember> familyMembers =
        await locator.familyRepository.queryForcesEmployees();
    ...
    // Update the grouped map and trigger a UI update
    setState(() {
      groupedForceFamilies = groupedFamilies;
    });
  ...
  Future<void> generatePdf() async {
    ...
    final List<FamilyMember> members = groupedForceFamilies.values
    ...
    if (members.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No Forces Employees found to generate PDF'),
          ...
          return pw.Container(
            ...
            child: pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Forces Employees - Employee Connect App",
                    style: pw.TextStyle(
          ...
          return [
            ...
            pw.Container(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Total Households: ${groupedForceFamilies.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                  pw.Text(
                    "Total Forces Employees: ${members.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                ],
            ...
            pw.ListView.builder(
              itemCount: groupedForceFamilies.length,
              itemBuilder: (context, index) {
                final householdNumber =
                    groupedForceFamilies.keys.toList()[index];
                final householdMembers = groupedForceFamilies[householdNumber]!;
    ...
    try {
      ...
      final currentDate = DateTime.now();
      final fileName =
          "Forces_Employees_Employee_Connect_${currentDate.day}-${currentDate.month}-${currentDate.year}.pdf";
  ...
  Widget build(BuildContext context) {
    // Get family count and member count after the state has been updated
    int familyCount = groupedForceFamilies.keys.length;
    int memberCount = groupedForceFamilies.values
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const NoDataFallback(
              ...
              message: 'There are no family members recorded under the Forces job category.',
            );
          }
