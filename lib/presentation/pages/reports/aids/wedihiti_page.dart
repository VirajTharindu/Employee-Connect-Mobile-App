...
class WedihitiFamiliesScreen extends StatefulWidget {
  const WedihitiFamiliesScreen({super.key});
...
class _WedihitiFamiliesScreenState extends State<WedihitiFamiliesScreen> {
  ...
  Future<void> _fetchWedihitiFamilies() async {
    ...
    final List<Map<String, dynamic>> familyMembersMap =
        await familyRepository.queryWedihitiFamilies();
  ...
  Future<void> generatePdf() async {
    ...
    final List<FamilyMember> members = groupedWedihitiFamilies.values
    ...
    if (members.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No Adult Aid recipients found to generate PDF'),
          ...
          return pw.Container(
            ...
            child: pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Adult Aid Recipients - Employee Connect App",
                    style: pw.TextStyle(
          ...
          return [
            ...
            pw.Container(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Total Households: ${groupedWedihitiFamilies.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                  pw.Text(
                    "Total Adult Aid Recipients: ${members.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                ],
            ...
            pw.ListView.builder(
              itemCount: groupedWedihitiFamilies.length,
              itemBuilder: (context, index) {
                final householdNumber =
                    groupedWedihitiFamilies.keys.toList()[index];
    ...
    try {
      ...
      final currentDate = DateTime.now();
      final fileName =
          "Adult_Aid_Recipients_Employee_Connect_${currentDate.day}-${currentDate.month}-${currentDate.year}.pdf";
  ...
  Widget build(BuildContext context) {
    int familyCount = groupedWedihitiFamilies.keys.length;
    int memberCount = groupedWedihitiFamilies.values
    ...
    return Scaffold(
      appBar: AppBar(
        title: Column(
          ...
          children: [
            const Text('Adult Aid receivers'),
            Text(
      ...
      body: groupedWedihitiFamilies.isEmpty
          ? const NoDataFallback(
              ...
              message: 'There are no family members recorded as receiving Wedihiti aid.',
            )
          : ListView.builder(
