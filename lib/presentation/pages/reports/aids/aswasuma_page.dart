...
class AswasumaFamiliesScreen extends StatefulWidget {
  const AswasumaFamiliesScreen({super.key});
...
class _AswasumaFamiliesScreenState extends State<AswasumaFamiliesScreen> {
  ...
  Future<void> _fetchAswasumaFamilies() async {
    final familyRepository = locator.familyRepository;
    final List<Map<String, dynamic>> familyMembersMap =
        await familyRepository.queryAswasumaFamilies(); // Query for Aswasuma families

    final List<FamilyMember> familiesWithAswasuma =
        familyMembersMap.map((map) => FamilyMemberModel.fromMap(map)).toList();
  ...
  Future<void> generatePdf() async {
    ...
    final List<FamilyMember> members = groupedAswasumaFamilies.values
    ...
    if (members.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No Aswasuma Aid recipients found to generate PDF'),
          ...
          return pw.Container(
            ...
            child: pw.Padding(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Aswasuma Aid Recipients - Employee Connect App",
                    style: pw.TextStyle(
          ...
          return [
            ...
            pw.Container(
              ...
              child: pw.Row(
                ...
                children: [
                  pw.Text(
                    "Total Households: ${groupedAswasumaFamilies.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                  pw.Text(
                    "Total Aswasuma Aid Recipients: ${members.length}",
                    style: pw.TextStyle(
                        font: ttfBold, fontSize: 12, color: PdfColors.green900),
                  ),
                ],
            ...
            pw.ListView.builder(
              itemCount: groupedAswasumaFamilies.length,
              itemBuilder: (context, index) {
                final householdNumber =
                    groupedAswasumaFamilies.keys.toList()[index];
    ...
    try {
      ...
      final currentDate = DateTime.now();
      final fileName =
          "Aswasuma_Aid_Recipients_Employee_Connect_${currentDate.day}-${currentDate.month}-${currentDate.year}.pdf";
  ...
  Widget build(BuildContext context) {
    int familyCount = groupedAswasumaFamilies.keys.length;
    int memberCount = groupedAswasumaFamilies.values
    ...
    return Scaffold(
      appBar: AppBar(
        title: Column(
          ...
          children: [
            const Text('Aswasuma Aid receivers'),
            Text(
      ...
      body: groupedAswasumaFamilies.isEmpty
          ? const NoDataFallback(
              title: 'No Recipients Found',
              message: 'There are no family members recorded as receiving Aswasuma aid.',
            )
          : ListView.builder(
